...
library(purrr)
source("/scratch/Shares/rinnclass/CLASS_2022/EM/CLASS_2022/util/encode_functions.R")
```
...
So we are going to combine our skills from 02_downloading data, 11_functions and now learning some API

Here is a snap shot of the data to be downloaded -- 
let's start by putting in browser and taking a look

First let's go see what we can find for HEPG2 RNAseq
...
Please move to the 17_ directory and make a fastq folder. Then run the following in that directory


```{bash downloading file list and sample sheet}
# We need two things:
# a samples file with information on samples
# a files file that has the path to fastq files on encode portal
...
# Now we need to copy files.txt into a new dir (inside 17_) called fastq
mkdir fastq
# now copy files.txt to fastq
cp files.txt fastq
# MAKE SURE YOUR TERMINAL IS IN THE RIGHT WORKING DIR.
pwd 
/17_/fastq/
# To download the fastq files for RNAseq we run this below:
...
Basically a highly specified language for interacting and retrieving the data you need.

The ENCODE API provides extensive documentation as to what data you can request and how to format that data to make your request. You can browse the possible requests in an interactive way using their interactive documentation:
...
# This is the same as running the function encode_file_info for each experiment
# in file_info. That we ran above:
dat <- encode_file_info("ENCSR541TIG")
...
# Check out "dat" in environment -- now file_info is a single entry with the dat data.frame inside.
