...
CLASS_2022/class_exeRcises/10_genome_features_ranges.Rmd
@boulderrinnlab
boulderrinnlab ready for class
Latest commit 079e14b 1 hour ago
History
1 contributor
486 lines (336 sloc) 14.3 KB
---
...
# 13,771 enteries
# look at env
# let's make a data-frame of what we just did above seperate for @from & @to
...
Idea
*********************
The promoters that had multiple peak overlaps are larger. 
```{r}
gencode_promoters6 <- promoters(gencode_gr[gencode_gr$type == "gene"], 
                               upstream = 6e3, 
                               downstream = 6e3)

promoter_overlaps6 <- findOverlaps(pol2_chip, gencode_promoters6)

length(promoter_overlaps6@to) 
length(promoter_overlaps6@from)
length(table(promoter_overlaps6@from))

gencode_promoters1 <- promoters(gencode_gr[gencode_gr$type == "gene"], 
                               upstream = 1e3, 
                               downstream = 1e3)

promoter_overlaps1 <- findOverlaps(pol2_chip, gencode_promoters1)

length(promoter_overlaps1@to) 
length(promoter_overlaps1@from)
length(table(promoter_overlaps1@from))
